...
// =====================================================
// INIT LINK REVIEWS & USER PROFILES TABLES
// =====================================================
...
function getRandomReplyTemplate(
  ...
  if (Array.isArray(automation?.public_reply_templates)) {
    templates = automation.public_reply_templates;
  } else if (typeof automation?.public_reply_templates === "string") {
    try {
      const parsed = JSON.parse(automation.public_reply_templates);
      if (Array.isArray(parsed)) {
        templates = parsed;
      }
    } catch {
      if (automation.public_reply_templates.trim()) {
        templates = [automation.public_reply_templates];
      }
    }
  }
...
function formatReplyText(template, username = "") {
  if (!template) return "";
  const rawUser = String(username || "").trim();
  const handle = rawUser && rawUser !== "unknown" ? `@${rawUser.replace(/^@/, "")}` : "";
  return template.replace(/@?\{username\}/gi, handle).trim();
}
...
async function updateAutomationStats(automationId) {
  ...
  try {
    ...
    console.log(`Stats updated for automation #${automationId} (+1 trigger) ✅`);
  } catch (error) {
    console.error("UPDATE AUTOMATION STATS ERROR:", error.message);
  }
}
...
async function recordActivityLog({
  ...
  try {
    ...
    console.log("Activity log recorded in public.activity_logs ✅");
  } catch (error) {
    try {
      await pool.query(
        `
      ...
      console.error("ACTIVITY LOG INSERT ERROR (non-blocking):", error.message);
    }
  }
}
...
// =====================================================
// VERIFY SUPABASE USER & AUTH MIDDLEWARE
// =====================================================

async function verifySupabaseUser(accessToken) {
  if (!accessToken || !SUPABASE_URL || !SUPABASE_API_KEY) return null;
  try {
    ...
    if (!response.ok || !user?.id) {
      console.error("SUPABASE USER VERIFY FAILED:", { status: response.status, response: user });
      return null;
    }
    return user;
  } catch (error) {
    console.error("SUPABASE USER VERIFY ERROR:", error.message);
    return null;
  }
}
...
function getSupabaseAccessTokenFromRequest(req) {
  ...
  return String(req.body?.access_token || req.query?.access_token || "").trim();
}
...
async function requireSupabaseUser(req, res, next) {
  ...
  const customUserId = String(req.headers["x-user-id"] || req.body?.user_id || "").trim();
...
async function getOwnedInstagramAccount(userId, accountId) {
...
// =====================================================
// AUTOMATION COLUMN TYPE & NORMALIZATION
// =====================================================

let automationReplyColumnType = null;

async function getAutomationReplyColumnType() {
  if (automationReplyColumnType) return automationReplyColumnType;
  try {
    const result = await pool.query(
      `
    ...
    if (
      ...
      automationReplyColumnType = "jsonb";
    } else {
      automationReplyColumnType = "array";
    }
  } catch (error) {
    console.error("AUTOMATION COLUMN TYPE CHECK ERROR:", error.message);
    automationReplyColumnType = "array";
  }
  return automationReplyColumnType;
}

function normalizeAutomationInput(body = {}) {
  ...
  if (!Array.isArray(publicReplyTemplates)) {
  ...
  if (enabledValue !== undefined) {
    if (typeof enabledValue === "string") {
      ...
      enabled = !["false", "0", "off", "no"].includes(normalizedEnabled);
    } else {
      enabled = Boolean(enabledValue);
    }
  }

  const rawDelay = body.delay_seconds ?? body.delaySeconds ?? 8;
  const delay_seconds = Math.min(Math.max(parseInt(rawDelay, 10) || 8, 3), 20);

  const rawHourly = body.hourly_limit ?? body.hourlyRateLimit ?? body.hourly_rate_limit ?? 80;
  const hourly_limit = Math.min(Math.max(parseInt(rawHourly, 10) || 80, 10), 120);
  ...
  const rawPreset = String(
    body.safety_speed_preset || body.safetySpeedPreset || "recommended"
  ).trim().toLowerCase();
...
function serializeAutomationForClient(account, automation) {
  const effective = buildEffectiveAutomationAccount(account, automation || {});
  const templates = Array.isArray(automation?.public_reply_templates)
    ? automation.public_reply_templates
    : [effective.public_reply || DEFAULT_PUBLIC_REPLY].filter(Boolean);

  const delay_seconds = Math.min(Math.max(parseInt(automation?.delay_seconds, 10) || 8, 3), 20);
  const hourly_limit = Math.min(Math.max(parseInt(automation?.hourly_limit, 10) || 80, 10), 120);
  const random_jitter_enabled = typeof automation?.random_jitter_enabled === "boolean" ? automation.random_jitter_enabled : true;
  const rawPreset = String(automation?.safety_speed_preset || "recommended").trim().toLowerCase();
  const safety_speed_preset = ["fast", "recommended", "very_safe", "custom"].includes(rawPreset) ? rawPreset : "recommended";
  ...
  return {
    ...
    public_reply_templates: templates,
    public_reply: templates[0] || effective.public_reply || DEFAULT_PUBLIC_REPLY,
    private_dm_message: automation?.private_dm_message ?? effective.private_text ?? DEFAULT_PRIVATE_TEXT,
    button_text: automation?.button_text ?? effective.button_title ?? DEFAULT_BUTTON_TITLE,
    channel_url: automation?.channel_url ?? effective.channel_url ?? DEFAULT_CHANNEL_URL,
    delay_seconds,
    delaySeconds: delay_seconds,
    hourly_limit,
    hourlyRateLimit: hourly_limit,
    random_jitter_enabled,
    randomDelayVariance: random_jitter_enabled,
    safety_speed_preset,
    safetySpeedPreset: safety_speed_preset,
    total_triggered: automation?.total_triggered || 0,
    last_triggered: automation?.last_triggered || null,
    enabled: automation?.enabled === undefined || automation?.enabled === null ? true : automation.enabled !== false,
    updated_at: automation?.updated_at || account.updated_at || null
  };
}

async function saveAutomationForOwnedAccount(account, input) {
  const replyColumnType = await getAutomationReplyColumnType();
  const replyValue = replyColumnType === "jsonb" ? JSON.stringify(input.publicReplyTemplates) : input.publicReplyTemplates;
  const replyExpression = replyColumnType === "jsonb" ? "$3::jsonb" : "$3";
...
function verifyOAuthState(state) {
  try {
    if (!state || !OAUTH_STATE_SECRET) return null;
    ...
    if (!decoded?.userId || !decoded?.expiresAt || Date.now() > decoded.expiresAt) return null;
    return decoded;
  } catch (error) {
    console.error("OAUTH STATE VERIFY ERROR:", error.message);
    return null;
  }
}
...
// =====================================================
// DATABASE HELPERS FOR INSTAGRAM ACCOUNTS
// =====================================================

async function getAccountByWebhookId(instagramUserId) {
  const result = await pool.query(
    `
    select *
    from instagram_accounts
...
async function getAccountByOAuthId(userId, oauthUserId) {
  const result = await pool.query(
    `
    select *
    from instagram_accounts
    ...
    limit 1
    `,
    [userId, String(oauthUserId)]
  );
  return result.rows[0] || null;
}

async function getAccountByUsername(userId, username) {
  const result = await pool.query(
    `
    select *
    from instagram_accounts
...
async function getAutomationForAccount(account) {
  if (!account?.id || !account?.user_id) return null;
  const result = await pool.query(
    `
    select *
...
function buildEffectiveAutomationAccount(account, automation) {
  const publicReply = getRandomReplyTemplate(automation, account.public_reply || DEFAULT_PUBLIC_REPLY);
  const privateText = String(automation?.private_dm_message || "").trim();
  const buttonTitle = String(automation?.button_text || "").trim();
  const channelUrl = String(automation?.channel_url || "").trim();

  return {
    ...account,
    public_reply: publicReply,
    private_text: privateText || account.private_text,
    button_title: buttonTitle || account.button_title,
    channel_url: channelUrl || account.channel_url
  };
}

async function saveOAuthAccount({ userId, oauthUserId, username, accessToken, expiresAt }) {
  if (!userId) throw new Error("Supabase userId is required.");
  let existing = await getAccountByOAuthId(userId, oauthUserId);
  if (!existing && username) {
    existing = await getAccountByUsername(userId, username);
  }
  ...
  if (existing) {
    const result = await pool.query(
      ...
      set
        ...
        access_token = $3,
        token_expires_at = $4,
        channel_url = coalesce(channel_url, $5),
        enabled = true,
        updated_at = now()
      where id = $6
      and user_id = $7
      returning *
      `,
      [String(oauthUserId), String(username || ""), accessToken, expiresAt, DEFAULT_CHANNEL_URL, existing.id, userId]
    );
    if (!result.rows[0]) throw new Error("Unable to update Instagram account.");
  ...
  const result = await pool.query(
    ...
    values ($1, $2, null, $3, $4, $5, $6, true, now(), now())
    returning *
    `,
    [userId, String(oauthUserId), String(username || ""), accessToken, DEFAULT_CHANNEL_URL, expiresAt]
  );
  return result.rows[0];
}

async function saveWebhookId(accountId, webhookInstagramId) {
  const result = await pool.query(
    `
    update instagram_accounts
    set instagram_user_id = $1, updated_at = now()
    where id = $2
    returning *
    `,
    [String(webhookInstagramId), accountId]
  );
  return result.rows[0] || null;
}

async function autoMapWebhookAccount(webhookInstagramId) {
  console.log("Trying webhook ID mapping:", webhookInstagramId);
  const result = await pool.query(
    `select * from instagram_accounts where enabled = true order by id asc`
  );
  const accounts = result.rows;
  if (accounts.length === 0) {
    console.error("No connected Instagram accounts.");
    return null;
  }
  ...
  for (const candidate of accounts) {
    ...
    try {
      ...
      const data = safeJsonParse(await response.text());
      if (!response.ok || !data.username) continue;

      const detectedUsername = normalizeUsername(data.username);
      const matchedAccount = accounts.find(
        account => normalizeUsername(account.username) === detectedUsername
      );
      if (!matchedAccount) continue;

      const updated = await saveWebhookId(matchedAccount.id, webhookInstagramId);
      console.log("Webhook account ID linked automatically ✅", {
        username: updated.username,
        webhookInstagramId: updated.instagram_user_id
      });
      return updated;
    } catch (error) {
      console.error("Webhook mapping attempt error:", error.message);
    }
  }
...
async function resolveWebhookAccount(webhookInstagramId) {
  const existing = await getAccountByWebhookId(webhookInstagramId);
  if (existing) return existing;
  return autoMapWebhookAccount(webhookInstagramId);
}
...
async function exchangeForLongLivedToken(shortToken) {
  try {
    ...
    if (!response.ok || !data.access_token) {
      console.error("LONG TOKEN FAILED:", data);
      return { ok: false, data };
    }
    const expiresIn = Number(data.expires_in || 5184000);
    const expiresAt = new Date(Date.now() + expiresIn * 1000);
    return { ok: true, accessToken: data.access_token, expiresAt, expiresIn };
  } catch (error) {
    console.error("LONG TOKEN ERROR:", error);
    return { ok: false, data: { error: error.message } };
  }
}

async function refreshAccountTokenIfNeeded(account) {
  if (!account || !account.access_token || !account.token_expires_at) return account;
  const now = Date.now();
  const expiry = new Date(account.token_expires_at).getTime();
  const sevenDays = 7 * 24 * 60 * 60 * 1000;

  if (expiry - now > sevenDays) return account;
  ...
  try {
    ...
    const data = safeJsonParse(await response.text());
    if (!response.ok || !data.access_token) {
      console.error(`TOKEN REFRESH FAILED @${account.username}:`, data);
      return account;
    }
    const expiresIn = Number(data.expires_in || 5184000);
    const expiresAt = new Date(Date.now() + expiresIn * 1000);
    ...
    const result = await pool.query(
      ...
      returning *
      `,
      [data.access_token, expiresAt, account.id]
    );
    console.log(`Token refreshed @${account.username} ✅`);
...
setInterval(async () => {
  try {
    ...
    for (const account of result.rows) {
      await refreshAccountTokenIfNeeded(account);
      await sleep(1500);
    }
  } catch (error) {
...
function getAccountSendHistory(accountId) {
  const key = String(accountId || "global").trim();
  const now = Date.now();
  const oneHourAgo = now - 3600000;
  const history = accountSendHistoryMap.get(key) || [];
  const activeHistory = history.filter(ts => ts > oneHourAgo);
  accountSendHistoryMap.set(key, activeHistory);
  return activeHistory;
}

function checkAccountRateLimit(accountId, hourlyLimit = 80) {
  const history = getAccountSendHistory(accountId);
  const limit = Math.min(Math.max(Number(hourlyLimit) || 80, 10), 120);
  const currentCount = history.length;
  if (currentCount < limit) {
    return { allowed: true, currentCount, hourlyLimit: limit, waitSeconds: 0 };
  }
  const oldest = history[0] || Date.now();
  const waitMs = Math.max(1000, (oldest + 3600000) - Date.now());
  return { allowed: false, currentCount, hourlyLimit: limit, waitSeconds: Math.ceil(waitMs / 1000) };
}

function recordAccountSend(accountId) {
  const key = String(accountId || "global").trim();
  const history = getAccountSendHistory(accountId);
  history.push(Date.now());
  accountSendHistoryMap.set(key, history);
}

function calculateNextDelayMs(delaySeconds = 8, jitterEnabled = true) {
  const base = Math.min(Math.max(Number(delaySeconds) || 8, 3), 20);
  let jitter = 0;
  if (jitterEnabled) {
    jitter = Math.round((Math.random() * 4 - 2) * 10) / 10;
  }
  const finalDelaySec = Math.max(1, Math.round((base + jitter) * 10) / 10);
...
async function fetchJsonWithRetry(url, options, label, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.error(`${label} FAILED (${response.status}):`, data);
  ...
  return { ok: false, status: 0, data: lastData };
}


// =====================================================
// INSTAGRAM REPLIES & CUSTOM DOMAIN VERIFICATION
// =====================================================

async function sendPublicReply(account, commentId, commenterUsername = "") {
  const rawMessage = account.public_reply || DEFAULT_PUBLIC_REPLY;
  const message = formatReplyText(rawMessage, commenterUsername);
...
async function sendPrivateButton(account, commentId, commenterUsername = "") {
  const channelUrl = await getVerifiedSafeUrlForAccount(account);
  const rawText = account.private_text || DEFAULT_PRIVATE_TEXT;
  const text = formatReplyText(rawText, commenterUsername);
  const buttonTitle = account.button_title || DEFAULT_BUTTON_TITLE;
...
async function sendPrivateFallback(account, commentId, commenterUsername = "") {
  const channelUrl = await getVerifiedSafeUrlForAccount(account);
  const rawText = account.private_text || DEFAULT_PRIVATE_TEXT;
  const text = formatReplyText(rawText, commenterUsername);
  const buttonTitle = account.button_title || DEFAULT_BUTTON_TITLE;
...
async function sendPrivateReply(account, commentId, commenterUsername = "") {
  const buttonResult = await sendPrivateButton(account, commentId, commenterUsername);
  if (buttonResult.ok) {
    console.log(`Private button sent @${account.username} ✅`);
    return buttonResult;
  }
  console.log("Button unavailable. Trying fallback.");
  return sendPrivateFallback(account, commentId, commenterUsername);
}


// =====================================================
// AUTOMATION QUEUE & COMMENT PROCESSING
// =====================================================
...
async function handleCommentAutomation(job) {
  ...
  // 1. Send Public Reply
  const publicResult = await sendPublicReply(effectiveAccount, job.commentId, job.commenterUsername);
  if (publicResult.ok) {
  ...
  // 2. Send Private DM
  const privateResult = await sendPrivateReply(effectiveAccount, job.commentId, job.commenterUsername);
  if (privateResult.ok) {
    console.log(`Private reply sent @${account.username} ✅`);
  } else {
    console.error(`PRIVATE REPLY FAILED @${account.username}:`, privateResult.data);
  }
  ...
  try {
    while (automationQueue.length > 0) {
      ...
      for (let i = 0; i < automationQueue.length; i++) {
        const candidateJob = automationQueue[i];
        const resolvedAccount = await resolveWebhookAccount(candidateJob.instagramUserId);
        if (!resolvedAccount) {
          eligibleIndex = i;
          break;
        }

        const accId = resolvedAccount.id;
        const nextAllowed = accountNextAllowedTimeMap.get(accId) || 0;
        if (now < nextAllowed) continue;

        const autoRow = await getAutomationForAccount(resolvedAccount);
        const hourlyLimit = autoRow?.hourly_limit ?? 80;
        const rateCheck = checkAccountRateLimit(accId, hourlyLimit);
      ...
      const [job] = automationQueue.splice(eligibleIndex, 1);
      try {
        const result = await handleCommentAutomation(job);
        if (result?.executed && result?.accountId) {
          recordAccountSend(result.accountId);
          const nextDelayMs = calculateNextDelayMs(result.delaySeconds, result.randomJitterEnabled);
          accountNextAllowedTimeMap.set(result.accountId, Date.now() + nextDelayMs);
          console.log(`Next execution for account #${result.accountId} allowed in ${(nextDelayMs / 1000).toFixed(1)}s`);
        }
      } catch (error) {
    ...
    if (automationQueue.length > 0 && AUTOMATION_ENABLED) {
      setTimeout(processAutomationQueue, 1500);
    }
  }
}

function enqueueCommentAutomation(job) {
  if (processedComments.has(job.commentId)) {
    console.log("Duplicate comment skipped:", job.commentId);
    return;
  }
  processedComments.add(job.commentId);
  if (processedComments.size > 5000) {
    processedComments.clear();
    processedComments.add(job.commentId);
  }
  automationQueue.push(job);
  console.log(`Comment queued ✅ Queue: ${automationQueue.length}`);
  setImmediate(processAutomationQueue);
}


// =====================================================
// GENERAL APP & HEALTH ROUTES
// =====================================================
...
app.get("/connect", (req, res) => {
  return res.status(400).send("Please connect Instagram from your logged-in ODD BOT dashboard.");
});

app.get("/health", async (req, res) => {
  try {
    const database = await pool.query("select now()");
    return res.status(200).json({
      success: true,
      database: "connected",
      database_time: database.rows[0].now,
      automation_enabled: AUTOMATION_ENABLED,
      queue: automationQueue.length
    });
  } catch (error) {
    return res.status(500).json({ success: false, database: "failed", error: error.message });
  }
});


// =====================================================
// DASHBOARD KEY AUTH & DASHBOARD API
// =====================================================

async function requireDashboardKey(req, res, next) {
  const authorization = String(req.headers.authorization || "").trim();
  const expectedAdmin = DASHBOARD_API_KEY ? `Bearer ${DASHBOARD_API_KEY}` : "";

  if (expectedAdmin && authorization === expectedAdmin) {
    req.oddBotDashboardAdmin = true;
    return next();
  }
  ...
  if (user?.id) {
    req.oddBotUser = user;
    req.oddBotDashboardAdmin = false;
    return next();
  }
...
app.get("/api/dashboard", requireDashboardKey, async (req, res) => {
  try {
    const dashboardUserId = req.oddBotUser?.id || null;
    const result = dashboardUserId
      ? await pool.query(
          `
          select id, instagram_user_id, username, enabled, created_at, updated_at, token_expires_at
          from instagram_accounts
          where user_id = $1
    ...
    const expiredAccounts = accounts.filter(account => account.status === "expired").length;
    ...
    return res.status(500).json({ success: false, error: "Dashboard data unavailable." });
  }
});


// =====================================================
// AUTOMATION API (LOAD & SAVE)
// =====================================================

app.get("/api/automation", requireSupabaseUser, async (req, res) => {
  try {
    const accountId = String(req.query?.account_id || req.query?.accountId || "").trim();
    if (!accountId) {
      return res.status(400).json({ success: false, error: "account_id is required." });
    }

    const account = await getOwnedInstagramAccount(req.oddBotUser.id, accountId);
    if (!account) {
      return res.status(404).json({ success: false, error: "Instagram account not found for this user." });
    }

    const automation = await getAutomationForAccount(account);
    return res.status(200).json({
      success: true,
      account: { id: account.id, username: account.username || "", enabled: account.enabled !== false },
      automation: serializeAutomationForClient(account, automation)
    });
  } catch (error) {
    console.error("LOAD AUTOMATION ERROR:", error);
    return res.status(500).json({ success: false, error: "Unable to load automation settings." });
  }
});

app.post("/api/automation", requireSupabaseUser, async (req, res) => {
  try {
    ...
    if (accountId && accountId !== 'primary') {
      account = await getOwnedInstagramAccount(req.oddBotUser.id, accountId);
    }
    ...
    const incomingUrl = input.channelUrl.trim();
    ...
    if (incomingUrl) {
      ...
      const platformInfo = identifyPlatform(incomingUrl);
      ...
      if (!platformInfo.isOfficial) {
        ...
        const existingReview = await pool.query(
          ...
          limit 1
          `,
          [req.oddBotUser.id, incomingUrl]
        );

        const reviewRow = existingReview.rows?.[0];
        ...
        if (!reviewRow || (reviewRow.review_status !== "approved" && reviewRow.status !== "approved")) {
          ...
          console.log(`Custom URL queued for admin review: ${incomingUrl} (Account: #${account.id})`);
        } else {
    ...
    const automation = await saveAutomationForOwnedAccount(account, saveInput);
...
app.get("/api/admin/link-reviews", requireAdminAuth, async (req, res) => {
  try {
    const status = req.query.status;
    let query = `select * from public.link_review_requests`;
    const params = [];
...
app.post("/api/admin/link-reviews/:id/approve", requireAdminAuth, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (review.account_id && review.user_id) {
      await client.query(
        `
        update public.automations
        set channel_url = $1, updated_at = now()
        where account_id = $2
        and user_id = $3
        `,
...
app.post("/api/admin/link-reviews/:id/reject", requireAdminAuth, async (req, res) => {
  const { id } = req.params;
...
// =====================================================
// WEBHOOK VERIFY & RECEIVE INSTAGRAM WEBHOOK
// =====================================================
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      for (const change of changes) {
        ...
        const commenterUsername = String(value.from?.username || "unknown");

        if (!instagramUserId || !commentId) continue;
        if (commenterId && commenterId === instagramUserId) {
          console.log("Own comment skipped.");
          continue;
        }
        if (!AUTOMATION_ENABLED) continue;
      ...
      for (const event of messaging) {
        ...
        console.log("Normal Instagram DM received:", {
...
// =====================================================
// INSTAGRAM OAUTH START & CALLBACK
// =====================================================

app.post("/auth/instagram/start", async (req, res) => {
  try {
    if (!INSTAGRAM_APP_ID) {
      return res.status(500).json({ success: false, error: "INSTAGRAM_APP_ID is missing." });
    }
    if (!SUPABASE_URL || !SUPABASE_API_KEY) {
      return res.status(500).json({ success: false, error: "Supabase backend configuration is missing." });
    }
    if (!OAUTH_STATE_SECRET) {
      return res.status(500).json({ success: false, error: "OAUTH_STATE_SECRET is missing." });
    }

    const accessToken = String(req.body?.access_token || "").trim();
    if (!accessToken) {
      return res.status(401).json({ success: false, error: "Supabase access token missing." });
    }

    const user = await verifySupabaseUser(accessToken);
    if (!user?.id) {
      return res.status(401).json({ success: false, error: "Invalid or expired ODD BOT login." });
    }

    console.log("Starting Instagram OAuth for user:", user.id);
    ...
    const params = new URLSearchParams({
      ...
      response_type: "code",
      state,
      scope: ["instagram_business_basic", "instagram_business_manage_comments", "instagram_business_manage_messages"].join(",")
    });

    const loginUrl = "https://www.instagram.com/oauth/authorize?" + params.toString();
    return res.status(200).json({ success: true, url: loginUrl });
  } catch (error) {
    console.error("START INSTAGRAM OAUTH ERROR:", error);
    return res.status(500).json({ success: false, error: "Unable to start Instagram authorization." });
  }
});
...
app.get("/auth/instagram/callback", async (req, res) => {
  try {
    ...
    if (!oauthState) {
      ...
      return res.status(400).send("Instagram connection session expired. Please return to ODD BOT and try again.");
    }

    const userId = String(oauthState.userId);
    console.log("Instagram OAuth callback for Supabase user:", userId);

    if (!INSTAGRAM_APP_ID || !INSTAGRAM_APP_SECRET) {
      return res.status(500).send("Instagram App ID or Secret missing.");
    }
    ...
    const shortData = safeJsonParse(await shortResponse.text());
    if (!shortResponse.ok || !shortData.access_token) {
      console.error("SHORT TOKEN FAILED:", shortData);
      return res.status(500).send("Instagram token exchange failed. Check Render Logs.");
    }

    const longResult = await exchangeForLongLivedToken(shortData.access_token);
    if (!longResult.ok) {
      return res.status(500).send("Long-lived token creation failed.");
    }

    const profileResponse = await fetch("https://graph.instagram.com/v26.0/me?fields=id,username", {
      headers: { Authorization: `Bearer ${longResult.accessToken}` }
    });

    const profile = safeJsonParse(await profileResponse.text());
    if (!profileResponse.ok || !profile.id) {
      console.error("PROFILE FAILED:", profile);
      return res.status(500).send("Instagram profile failed.");
    }

    const savedAccount = await saveOAuthAccount({
      userId,
      oauthUserId: String(profile.id),
      username: String(profile.username || ""),
      accessToken: longResult.accessToken,
      expiresAt: longResult.expiresAt
    });

    console.log("Instagram account saved:", {
      username: savedAccount.username,
      oauthUserId: savedAccount.oauth_user_id,
      userId: savedAccount.user_id
    });

    const subscriptionBody = new URLSearchParams();
    subscriptionBody.append("subscribed_fields", "comments,messages");

    const subscriptionResponse = await fetch("https://graph.instagram.com/v26.0/me/subscribed_apps", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${longResult.accessToken}`,
        "Content-Type": "application/x-www-form-urlencoded"
      },
      body: subscriptionBody
    });

    const subscriptionData = safeJsonParse(await subscriptionResponse.text());
    console.log("Webhook subscription:", subscriptionData);

    if (!subscriptionResponse.ok) {
      return res.status(500).send("Account saved, but webhook subscription failed. Check Render Logs.");
    }
...
<p>You can close this page and return to the dashboard.</p>
    ...
    return res.status(500).send("Instagram connection failed. Check Render Logs.");
  }
});
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log("ODD BOT MULTI-USER MODE ENABLED ✅");
  console.log("RANDOM REPLY TEMPLATES & TAGGING ENABLED ✅");
  console.log("STATS & ACTIVITY LOGS INTEGRATION ENABLED ✅");
  console.log("AUTO WEBHOOK ID MAPPING ENABLED ✅");
  console.log("PER-ACCOUNT RATE LIMITS & HUMAN JITTER ENGINE ENABLED ✅");
});
